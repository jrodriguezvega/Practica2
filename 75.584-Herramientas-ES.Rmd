...
medidas <- data.frame(colnames(df_train[,v_var_cuant]),round(v_var,4), round(v_sd,4), round(v_pvalue_shapiro,4))

colnames(medidas) <- c("attr", "varianza", "desviación estándar", "shapiro p-value")
medidas_dispersion
```
...
fligner.test(Age ~ SibSp, data = df_norm)
fligner.test(Age ~ Parch, data = df_norm)
fligner.test(Age ~ Fare, data = df_norm)
fligner.test(SibSp ~ Parch, data = df_norm)
fligner.test(SibSp ~ Fare, data = df_norm)
fligner.test(Parch ~ Fare, data = df_norm)
```

Se verifica que todas las variables dos a dos cumplen heterocedasticidad.
