...
* La variable *Age* posee casi un 20% de valores nulos; en este caso concreto, si bien es una cifra considerable, la significancia de este atributo puede ser relevante y, por tanto, se mantiene. En un análisis posterior, se determinará si se infieren estos valores o no y se verá cómo afecta a los distintos estadísticos. Por ejemplo, una opción podría ser sustituir los valores por la media/mediana en tanto que exista un comportamiento de normalidad.
...
Existen otro tipo de conversiones que se podrían hacer de los datos, como la normalización de las variables numéricas entre los valores [0,1] mediante transformaciones *min-max* o la normalización *z-score* que resta la media a la variable y la divide por su desviación estándar.
...
Dado que aún no conocemos la distribución de nuestros atributos no se van a realizar en este momento, dejando este tipo de conversión en los datos para más adelante según veamos la normalidad y homocedasticidad de las variables.
...
Una vez más, vamos a esperar a realizar un análisis sobre la distribución de las variables, que nos permitirá mejorar la completitud y calidad de nuestros datos realizando el tratamiento sobre valores perdidos y posteriormente las mencionadas transformaciones.

Como paso previo, y también necesario en el proceso de limpieza del juego de datos, se va a realizar un análisis de *outliers*.
...
Se verifica que todas las variables dos a dos cumplen heterocedasticidad al tener todas un p-value inferior al nivel de significancia (0.05).
